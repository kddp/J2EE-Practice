...
import com.uttara.test.AddContactBean;
import com.uttara.test.EditBean;
import com.uttara.test.JDBCHelper;
import com.uttara.test.LoginBean;
import com.uttara.test.RegBean;
...
public class MvcModel {
	...
	public String doRegistration(RegBean rb)
	{
		...
		try {
			...
			ps_sel=c.prepareStatement("Select email from registerTable");
			 ...
			 ps_ins=c.prepareStatement("insert into registerTable (name,email,pword,rpword)values(?,?,?,?)");
	...
	public String doLogin(LoginBean lb) {
		...
		try {
			...
			System.out.println("database server started");
			ps_sel=c.prepareStatement("Select pword from registerTable where email=?");
	...
	public String doEdit(EditBean eb, String em1) {
		...
		try {
			...
			System.out.println("database server started");
			ps_sel=c.prepareStatement("update registerTable set name=?,pword=?,rpword=? where email=?");
	...
	public EditBean getUserDetails(String email) {
		...
		try
		{
			...
			ps_sel = con.prepareStatement("select * from registerTable where email = ?");
			ps_sel.setString(1, email);
			ps_sel.execute();
